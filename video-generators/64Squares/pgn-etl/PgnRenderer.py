...
def send_video():
    scp_val = (os.system("scp ./vids-to-send/" + file_string + " pi@10.0.0.20:/mnt/64-squares-vids-to-upload/" + file_string))
    if scp_val == 0:
        # sent = True
        r.hset(x, "sent", 1)
...
def send_json():
    ...
    url = "http://10.0.0.20:8080/64-squares/add_video"  # Prod @todo move outside of loop, improve efficiency
...
# validate_redis()
recent_idx = int(r.get("64-squares-recent-index"))  # 275474 total videos # starting index for 64sqrs is
for x in reversed(range(1, recent_idx - NUM_OF_WORKERS)):
    ...
    r.decr("64-squares-recent-index")  # make method for resetting stuff?
    EVENT = ""
