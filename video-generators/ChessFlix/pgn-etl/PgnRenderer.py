...
FRAMERATE = 30
MOVE_TIME = 0.9  # seconds between moves
...
def send_video():
    scp_val = (os.system("scp ./vids-to-send/" + file_string + " pi@10.0.0.20:/mnt/chess-flix-vids-to-upload/" + file_string))
    if scp_val == 0:
        # sent = True
        r.hset(x, "sent", 1)
...
def send_json():
    ...
    url = "http://10.0.0.20:8080/chess-flix/add_video"  # Prod
...
validate_redis()
for x in range(1, r.dbsize() - 2):  # -3 bc validation, fail_list and videoQueue keys, then add 1 bc last value doesn't loop
