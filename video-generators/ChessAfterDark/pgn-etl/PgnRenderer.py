...
def send_video():
    scp_val = (os.system("scp ./vids-to-send/" + file_string + " pi@10.0.0.20:/mnt/chess-after-dark-vids-to-upload/" + file_string))
    if scp_val == 0:
        # sent = True
        r.hset(x, "sent", 1)
...
def send_json():
    ...
    url = "http://10.0.0.20:8080/chess-after-dark/add_video"  # Prod @todo move outside of loop, improve efficiency
    ...
    data = {
        ...
        "playlist": "Infinite Chess" if EVENT == "" else EVENT,  # get event name
        "thumbnail": " "
    }
...
# validate_redis()
recent_idx = int(r.get("chess-after-dark-recent-index"))  # 4233 in chessflix
for x in reversed(range(1, recent_idx - NUM_OF_WORKERS)):
    ...
    r.decr("chess-after-dark-recent-index")  # make method for resetting stuff?
    EVENT = ""
