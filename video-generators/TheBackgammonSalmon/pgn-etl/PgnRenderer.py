...
FRAMERATE = 30
MOVE_TIME = 1.3  # seconds between moves
...
def send_video():
    scp_val = (os.system("scp ./vids-to-send/" + file_string + " pi@10.0.0.20:/mnt/backgammon-salmon-vids-to-upload/" + file_string))
    if scp_val == 0:
        # sent = True
        r.hset(x, "sent", 1)
...
def send_json():
    ...
    url = "http://10.0.0.20:8080/backgammon-salmon/add_video"  # Prod @todo move outside of loop, improve efficiency
...
# validate_redis()
recent_idx = int(r.get("backgammon-salmon-recent-index"))  # 275474 total videos
for x in range(recent_idx - NUM_OF_WORKERS, 275474):
    ...
    r.decr("backgammon-salmon-recent-index")  # make method for resetting stuff?
    EVENT = ""
